...
begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          temp := 'nur ' + temp + ' radioakt. Isotop(e)';
        end;
        Cells[1,4] := temp;
      ...
      if DM.ETNukl.Locate('iZA',iza,[]) and
        (DM.ETNukl.FieldByName('Masse').AsFloat <> 0) then
        Cells[1,9] := FloatToStrF(DM.ETNukl.FieldByName('Masse').AsFloat / 1000 / 1000,ffGeneral,6,2)
      else Cells[1,9] := '';
  ...
  for i := 30 to 51 do   // Elektronen
    if DM.ETChem.Fields[i].AsInteger <> 0 then
      frmMain.GridElektron.Cells[i-29,1] := IntToStr(DM.ETChem.Fields[i].AsInteger)
    else frmMain.GridElektron.Cells[i-29,1] := '';
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      for i := 1 to RowCount - 1 do ColorCell[1,i] := clInfoBk;
      ...
      Cells[0,13] := 'Ionisierungsenergie in kcal/g-mol';
      ...
      Cells[0,23] := 'Verdampfungswärme in kcal/g-atom';
      Cells[0,24] := 'Schmelzwärme in kcal/g-atom';
      Cells[0,25] := 'Wärmeleitfähigkeit in cal/cm²/cm/°C/sec';
      Cells[0,26] := 'spezifische Wärme in cal/g/°C';
    ...
    GridElektron.HintCell[1,0] := 'Elektronen in der äußersten besetzten Schale';
    ...
    begin
      if i < 3 then GridElektron.ColWidths[i] := 26
      else GridElektron.ColWidths[i] := 27;
...
begin
  ...
  begin
    ...
    begin
      case RG.ItemIndex of
        3..9,11..25,27,28:
          if ((DM.ETChem.FieldByName(Spalte).DataType = ftFloat) and (DM.ETChem.FieldByName(Spalte).AsFloat >= Min)) or
            ((DM.ETChem.FieldByName(Spalte).DataType = ftInteger) and (DM.ETChem.FieldByName(Spalte).AsInteger >= Min)) then
          begin
            if DM.ETChem.FieldByName(Spalte).DataType = ftFloat then
              if not Wahr then
                i := 255 - Trunc(((DM.ETChem.FieldByName(Spalte).AsFloat / Max) * 100) * 2.55)
              else i := Trunc(((DM.ETChem.FieldByName(Spalte).AsFloat / Max) * 100) * 2.55);
...
begin
  ...
  begin
    case Key of
      VK_LEFT: if (PXneu = 1) and (PYneu <= 7) then PXneu := 18
               else if (PXneu = 18) and (PYneu = 1) then PXneu := 1
               else if (PXneu = 13) and ((PYneu = 2) or (PYneu = 3)) then PXneu := 2
               else if (PXneu = 1) and (PYneu >= 9) then PXneu := 17
               else if (PXneu = 3) and (PYneu = 9) then begin PXneu := 2; PYneu := 6; end
               else if (PXneu = 3) and (PYneu = 10) then begin PXneu := 2; PYneu := 7; end
               else if (PXneu = 4) and (PYneu = 6) then begin PXneu := 17; PYneu := 9; end
               else if (PXneu = 4) and (PYneu = 7) then begin PXneu := 17; PYneu := 10; end
               else Dec(PXneu);
      VK_RIGHT: if (PXneu = 18) and (PYneu <= 7) then PXneu := 1
                else if (PXneu = 1) and (PYneu = 1) then PXneu := 18
                else if (PXneu = 2) and ((PYneu = 2) or (PYneu = 3)) then PXneu := 13
                else if (PXneu = 17) and (PYneu = 9) then begin PXneu := 4; PYneu := 6; end
                else if (PXneu = 17) and (PYneu = 10) then begin PXneu := 4; PYneu := 7; end
                else if (PXneu = 2) and (PYneu = 6) then begin PXneu := 3; PYneu := 9; end
                else if (PXneu = 2) and (PYneu = 7) then begin PXneu := 3; PYneu := 10; end
                else Inc(PXneu);
      VK_UP: if ((PXneu = 1) or (PXneu = 18)) and (PYneu = 1) then PYneu := 7
             else if (PXneu = 2) and (PYneu = 2) then PYneu := 7
             else if (PXneu >= 13) and (PXneu <= 17) and (PYneu = 2) then PYneu := 10
             else if ((PXneu >= 3) and (PXneu <= 12)) and (PYneu = 4) then PYneu := 10
             else if (PXneu = 3) and (PYneu = 9) then begin PXneu := 2; PYneu := 7; end
             else if (PXneu >= 3) and (PXneu <= 17) and (PYneu = 9) then PYneu := 7
             else Dec(PYneu);
      VK_DOWN: if ((PXneu = 1) or (PXneu = 18)) and (PYneu = 7) then PYneu := 1
               else if (PXneu = 2) and (PYneu = 7) then PYneu := 2
               else if (PXneu >= 4) and (PXneu <= 18) and (PYneu = 7) then PYneu := PYneu + 2
               else if (PXneu = 3) and (PYneu = 5) then begin PXneu := 3; PYneu := 9; end
               else if (PXneu >= 3) and (PXneu <= 12) and (PYneu = 10) then PYneu := 4
               else if (PXneu >= 13) and (PXneu <= 17) and (PYneu = 10) then PYneu := 2
               else Inc(PYneu);
    end;
    GridPSE.Repaint;
