...
uses
  ...
  JvExComCtrls, JvStatusBar, JvRadioGroup, EasyTable, JvProgressBar;
...
type
  TfrmMain = class(TForm)
    ...
    procedure TVKeyUp(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure GridPSEKeyUp(Sender: TObject; var Key: Word;
      Shift: TShiftState);
  private
  ...
  public
    ...
    TVNukIdx,TVRadIdx,bRad,anzrad: Integer;
    Spalte: String;
    Min,Max: Double;
    VonF,ZuF: TColor;
...
begin
  ...
  //JvProgressBar1.Parent := StatusBar;
  //JvProgressBar1.Align := alClient;
  NukF[0] := clSilver;
  NukF[1] := 16678784;  // ß-
  NukF[2] := 32767;     // ß+
  NukF[3] := 43582;     // p      (ly)
...
begin
  ...
  begin
    ...
    if (row > 0) and ClearGridRadDat then RadDaten(HintStr) else ClearGridRadDat;
  end
...
begin
  ...
  begin
    ...
    ShowMessage('Um Bereiche des Diagramms detailliert zu betrachten' + #13#10 +
                'ziehen Sie einfach mit gedrückter linker Maustaste' + #13#10 +
                'einen beliebigen Bereich im Diagramm nach rechts' + #13#10 +
                'unten und lassen die Maustaste wieder los.' + #13#10 + #13#10 +
                'Mit den Buttom ''Lupe rückgängig'' können Sie wieder' + #13#10 +
...
begin
  ...
  if (bRad = 1) and ClearGridRad then
    RadReihe(iza)
  else if (bRad = 2) and ClearGridRad then
  begin
  ...
  else if (bRad = 3) and ClearGridRad then RadBack(iza);
  Gauge.Position := 0;
...
begin
  ...
  begin
    ...
    Screen.Cursor := crHourGlass;
    ClearGridRadDat;
    if (bRad < 3) and ClearGridRad and RadReihe(iza) then
    begin
    ...
    end else if ClearGridRad then RadBack(iza);
    Gauge.Position := 0;
    Screen.Cursor := crDefault;
  end;
end;
