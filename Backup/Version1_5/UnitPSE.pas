  ...
  with frmMain.GridPSE.Canvas do
    if DM.ETChem.Locate('iZA',iza,[]) then
      ...
      begin
        ...
        if not Wahr and (Pos(',',temp) > 0) then
        begin
          L1 := StrToInt(Copy(temp,1,Pos(',',temp)-1))-1;
...
begin
  ...
  begin
    ...
    c := StrToInt(Copy(temp,1,Pos(',',temp)-1));
    Delete(temp,1,Pos(',',temp));
    r := StrToInt(temp);
    if r > 7 then Inc(r);
...
begin
  ...
  begin
    ...
    begin
      ...
      if not DM.ETChem.FieldByName('AnzStabIso').IsNull then
        CellAsInt[1,5] := DM.ETChem.FieldByName('AnzStabIso').AsInteger
      else Cells[1,5] := '0';
      if not DM.ETChem.FieldByName('AnzInstabIso').IsNull then
        CellAsInt[1,6] := DM.ETChem.FieldByName('AnzInstabIso').AsInteger
      else Cells[1,6] := '0';
