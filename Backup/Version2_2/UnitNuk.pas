  ...
  with frmMain.GridRadDat do
    ...
    begin
      ...
      repeat
        ...
        iza := DM.ETDMI.FieldByName('DAUGHTER').AsInteger;
        farbe := DM.ETDMI.FieldByName('RTYP1').AsInteger;
        ...
        if DM.ETDMI.FieldByName('BR').AsFloat <> 0 then
          Cells[1,i] := FloatToStr(DM.ETDMI.FieldByName('BR').AsFloat);
        if iza = - 60 then
        begin
          CellFont[2,i].Color := clBlack;
          Cells[2,i] := '?'
        end else
          Cells[2,i] := DM.GetNukBez(DM.ETDMI.FieldByName('DAUGHTER').AsInteger,True);
      until DM.ETDMI.FindNext = False;
      DM.ETDMI.Filter := '';
    end;
  Result := True;
