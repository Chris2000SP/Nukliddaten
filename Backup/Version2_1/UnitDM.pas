...
type
  ...
  public
    ...
    ETNukl,ETDM,ETDMI,ETName,ETChem,ETRZA,ETA,ETB,ETG,ETXI,ETXR,ETAug: TEasyTable;
    DSNukl,DSDM,DSDMI,DSName,DSChem,DSA,DSB,DSG,DSXI,DSXR,DSAug: TDataSource;
...
implementation
  ...
  uses UnitMain, UnitNukFunc, UnitPSE, UnitNuk, Aligrid, Grids, icongrid;
    ...
    repeat
      ...
      begin
        if stabil and (ETNukl.FieldByName('Tsek').AsFloat = -99) then
          Inc(anz)
        else if not stabil and (ETNukl.FieldByName('Tsek').AsFloat <> -99) and
...
begin
  ...
  if OpenTab(ETNukl,'Nukl','ByiZA') and OpenTab(ETName,'NukNa','ByNr') and
    ...
    OpenTab(ETXR,'XRay','') and OpenTab(ETAug,'Auger','') then
  begin
...
begin
  ...
  begin
    ...
    repeat
      ...
      if (d < 9) and (ETDMI.FieldByName('RTYP1').AsInteger <> 6) and
        (ETDMI.FieldByName('RTYP2').AsInteger <> 6) then
...
begin
  ...
  begin
    frmMain.lblFilter.Caption := 'A1-N1 muss größer oder gleich A2-N2 sein';
    ...
    repeat
      ...
      begin
        ...
        begin
          ...
          if ET.Name = 'ETG' then ZeigeBeta(i,iza,ETG);
  ...
  //begin
    ...
    repeat
      ...
      begin
        ...
        xn := ETNukl.FieldByName('N').AsInteger;
        ...
        if DM.ETNukl.FieldByName('El').AsFloat <> 0 then
          Cells[5,i] := FloatToStr(DM.ETNukl.FieldByName('El').AsFloat);
        Cells[6,i] := DM.ETNukl.FieldByName('JPi').AsString;
