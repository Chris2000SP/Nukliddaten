...
begin
  ...
  begin
    ...
    frmMain.GridPSE.HintCell[c,r] := IntToStr(Z) + #32 + Sym + ' = ' +
      DM.NukName(Sym);
    {frmMain.GridSuchPSE.Cells[c,r-1] := Sym;
    frmMain.GridSuchPSE.HintCell[c,r-1] := frmMain.GridPSE.HintCell[c,r];}
    Result := True;
...
begin
  ...
  begin
    ...
    begin
      ...
      {if DM.ETChem.FieldByName('AnzStabIso').AsInteger <> 0 then
        Cells[1,5] := IntToStr(DM.ETChem.FieldByName('AnzStabIso').AsInteger)
      else Cells[1,5] := '';
      if DM.ETChem.FieldByName('AnzInstabIso').AsInteger <> 0 then
        Cells[1,6] := IntToStr(DM.ETChem.FieldByName('AnzInstabIso').AsInteger)
      else Cells[1,6] := '';}
