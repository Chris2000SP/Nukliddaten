...
uses Windows, Types, Classes, SysUtils, Math, Graphics, Dialogs, WinInet;
  ...
  function  DBDaten(db: Integer): String;
  ...
  procedure MalePfeil(Can: TCanvas;Col: TColor;SLange,Bet: Byte;
  ...
  function  MakeHTML(Titel: String;L: TStrings): String;
  ...
  function  Runde(Value: Double;Rto: Integer): String;
  Procedure RotText(Rot,x,y: Integer;aText: String;aCanvas: TCanvas;B: Boolean);
  function  RTyp2C(rt: Double): Integer;
...
begin
  case db of
    1: Result := 'Daten aus TORI-DB';
    2: Result := 'Daten aus dec-ENDF-VII0';
    3: Result := 'Daten aus JEFF311RDD_ALL';
    4: Result := 'Daten aus ENSDF 2006';
...
begin
  ...
  //2.Punkt
  Punkte[1].X := round(P2.X - sLange * cos(GetDEG(Alpha - Bet div 2)));
  Punkte[1].Y := round(P2.Y + sLange * sin(GetDEG(Alpha - Bet div 2)));
  //3.Punkt
  Punkte[2].X := round(P2.X - sLange * cos(GetDEG(Alpha + Bet div 2)));
  Punkte[2].Y := round(P2.Y + sLange * sin(GetDEG(Alpha + Bet div 2)));
  //Pfeil zeichnen
  if Filled then
    Can.Polygon(Punkte)
  else
...
begin
  ...
  Result := '<HTML>' +
    ...
    '<FONT SIZE="4" COLOR="NAVY"><B>' + L.Strings[0] + '<BR>' +
    '<BR>' + '<FONT SIZE="3">';
  for i := 1 to L.Count-1 do
    Result := Result + L.Strings[i] + '<BR>';
  {Result := Result + '<h2><a href="weiter">weiter</a></h2>' +
    '</CENTER></BODY></HTML>';}
...
begin
  ...
  begin
    ...
    if hk <> '' then TextOut(R.Left+hi,R.Top+th,hk);
    if uk <> '' then
    begin
      Font.Style := [fsItalic];
      Font.Size := 8;
  ...
  begin
    ...
    case i of
      1: begin  // Beta-
           ...
           p2.X := ToR.Right-10;
           p2.Y := ToR.Bottom-10;
           MalePfeil(Canv,clPurple,5,50,True,p1,p2);
...
var
  ...
  temp: String;
begin
       if Value < 0.000000001 then d := Value
  else if Value < 0.00000001 then d := RoundTo(Value,-9)
  else if Value < 0.0000001 then d := RoundTo(Value,-8)
  else if Value < 0.000001 then d := RoundTo(Value,-7)
  else if Value < 0.00001 then d := RoundTo(Value,-6)
  else if Value < 0.0001 then d := RoundTo(Value,-5)
  else if Value < 0.001 then d := RoundTo(Value,-4)
  else if Value < 0.01 then d := RoundTo(Value,-3)
  else if Value < 0.1 then d := RoundTo(Value,-2)
  else if Value < 1 then d := RoundTo(Value,-1)
  else d := RoundTo(Value,Rto);
  temp := FloatToStr(d);
  if (d < 1) and (d >= 0.001) and (Pos('E',temp) = 0) then
    Result := FloatToStr(d)
  else if (d < 1) and (d >= 0.0001) then
  begin
    temp := FloatToStrF(d,ffExponent,1,1);
    if (Pos(DM.DezS+'0',temp) > 0) and (Pos('E',temp) > 0) then
      Delete(temp,Pos(DM.DezS,temp),2);
    Result := temp;
  end else Result := FloatToStr(d);
end;
...
Begin
  ...
  farbe := aCanvas.Font.Color;
  Try 
    aCanvas.Font.Charset := ANSI_CHARSET; //DEFAULT_CHARSET;
    aCanvas.Font.Name := 'Arial'; //Tahoma'; // Muss TrueType sein
    aCanvas.Font.Size := 8;
    aCanvas.Font.Style := [fsBold];
    if B then aCanvas.Font.Color := clNavy else aCanvas.Font.Color := clskyBlue;
    GetObject(aCanvas.Font.Handle, SizeOf(aFt), @Aft);
    aFt.lfEscapement := 10 * Rot;  // Rotate ist der Winkel in Grad
...
begin
  ...
  begin
    ...
    TextOut(R.Left+tw,R.Top+th+4,tu1);
    SymFont(Canv,False);
    tw := tw + TextWidth(tu1)+2;
    Font.Size := 8;//10;
